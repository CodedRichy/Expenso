...
class LedgerDelta {
  ...
  @override
  String toString() =>
      'LedgerDelta($memberId: ${delta.amountMinor} ${delta.currencyCode} for $expenseId)';
}
...
List<LedgerDelta> toLedgerDeltas(
  ...
  for (final entry in expense.payerContributionsByMemberId.entries) {
    netByMember[entry.key] =
        (netByMember[entry.key] ?? 0) + entry.value.amountMinor;
  }
  ...
  assert(sum == 0, 'LedgerDelta sum must be exactly zero, got $sum');
...
List<LedgerDelta> expenseToLedgerDeltasLegacy({
  ...
  for (final entry in splitAmountsById.entries) {
    ...
    splitAmountsByIdMinor[entry.key] = MoneyConversion.parseToMinor(
      entry.value,
      currencyCode,
    ).amountMinor;
  }
