...
| 2026-03-01 | "Sent ₹750 to Prasi to clear my debt" / "Dinner wasn 3000 but Sam's dessert was 400" / "Jordan paid like last time" | LLM historically generated confident logic for unresolved splits, failed basic remainder arithmetic, and hallucinated settlements into standalone expenses. | Introduced `CONFIDENCE CRITERIA & STRICT RULES` block: Rule A (Integrity check limits history to constrained), Rule B (Exactness requirement), Rule C (Participant guard vs excluding), Rule D (Settlements reject parsing). Also implemented the "Remainder Rule" mathematically inside prompt and added defensive programmatic GAP validations inside Dart loops to intercept corrupted exact splits where LLM hallucinates remainder chunks. |
| (future)   | … | … | … |
